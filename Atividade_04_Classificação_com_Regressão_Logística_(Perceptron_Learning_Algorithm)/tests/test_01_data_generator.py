...
class TestDataGenerator(TestBaseLinearModel):
    ...
    def test_x_mean_at_2nd_quarter(self):
        expected = -0.50
        actual = np.mean(self.X[250:500, 1])
        self.assertAlmostEqual(actual, expected, delta=0.1)
    ...
    def test_x_mean_at_3rd_quarter(self):
        expected = 0.50
        actual = np.mean(self.X[500:750, 1])
        self.assertAlmostEqual(actual, expected, delta=0.1)
